...
# Create Orders
order_1st = Order.find_or_create_by!(user_id: user.id, status: 'ordered')
order_2nd = Order.find_or_create_by!(user_id: user.id, status: 'ordered')
...
Product.find_or_create_by!(order_id: order_2nd.id, code: '002', name: 'Personalised cufflinks', price: 45.00)
